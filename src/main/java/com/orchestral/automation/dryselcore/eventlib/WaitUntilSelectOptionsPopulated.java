...
public class WaitUntilSelectOptionsPopulated extends TestEvent {

	public WaitUntilSelectOptionsPopulated() {
	}

	@Override
	public void fire(final WebDriver webDriver, final String xPath, final String value) {
		final ExpectedCondition<Boolean> optionPopulated = new ExpectedCondition<Boolean>() {
			@Override
			public Boolean apply(final WebDriver wd) {
				return (new Select(wd.findElement(By.xpath(xPath))).getOptions().size() > 1);
			}
		};

		final int tries = 10;
		for (int thisTry = 1; thisTry <= tries; thisTry++) {
			final Boolean result = optionPopulated.apply(webDriver);
		...
		throw new TimeoutException("Failed to wait for options to be selected.");
	}
}
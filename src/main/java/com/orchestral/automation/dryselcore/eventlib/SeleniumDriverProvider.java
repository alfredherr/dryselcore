...
public class SeleniumDriverProvider {
	...
	public static void setWebDriver(final WebDriver webDriver) {
		SeleniumDriverProvider.webDriver = webDriver;
	}

	private static void initializeWebDriver() {
			System.setProperty("webdriver.gecko.driver", "src/test/resources/drivers/geckodriver");
			SeleniumDriverProvider.setWebDriver(new FirefoxDriver());
			SeleniumDriverProvider.getWebDriver().manage().timeouts().implicitlyWait(2, TimeUnit.MINUTES);
	}
	...
	public static synchronized void tearDown() {
		try {
			...
			if (webDriver != null && webDriver.getTitle() != null) {
				SeleniumDriverProvider.webDriver.quit();
				// this is required to cleanup the instance.
				SeleniumDriverProvider.webDriver = null;
			}
